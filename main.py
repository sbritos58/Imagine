...
class ContrasenaOlvidada(Screen):
     def __init__(self, **kwargs):
        super(ContrasenaOlvidada, self).__init__(**kwargs)
        def buscarcontrasena(self,email):
            self.text = email
            db = sqlite3.connect('base.db')
            c = db.cursor()
            c.execute('select contrasena from organizacion  where email=="%s"'%(email))  
            contra=str(c.fetchone()[0])
            print(contra)
            if contra is not None:
                self.ids["contrasenaperdida"].text=contra
            else:
                print("no exite")
            db.commit()
            db.close()
